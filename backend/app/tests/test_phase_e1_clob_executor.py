...
# SKIP reason (audit 2026-06): the executor order/credential surface below was
# rebuilt by the "live v2" refactor onto py_clob_client_v2 — an SDK that was never
# delivered (phantom module; see clob_executor.py). place_order(price=/size_shares=),
# ensure_api_credentials() and the constructor privkey validation no longer exist,
# and the current market-order path is not exercisable in dry-run/CI. Skipped (not
# deleted) so the intended surface stays on record; un-skip when live is rebuilt on
# a real SDK. The still-valid surface (cancel_all, status, build_executor_from_env,
# pUSD wrapper) stays active below.
_LIVE_V2_VAPORWARE = (
    "live-v2 executor path targets the undelivered py_clob_client_v2 SDK; "
    "un-skip when rebuilt on a real SDK."
)
...
@pytest.mark.skip(reason=_LIVE_V2_VAPORWARE)
