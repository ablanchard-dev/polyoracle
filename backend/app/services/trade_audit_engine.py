...
class TradeAuditEngine:
    ...
    def _decide(
        ...
        # Reverted 2026-06 — the abandoned live-v2 merge (d1d7e05, "risk_engine
        # min_copyable_edge neutralisé") rerouted PAPER_TRADE onto copyable_edge as
        # the sole master gate, removing the orderbook/spread/liquidity/quality and
        # paper-vs-live gates. That broke hard safety invariants (paper ≤ live shadow,
        # the tq>=70 quality floor, the strict/opt-in research gate). Restored to the
        # gated paper-bypass below, which the B13/audit spec tests pin. The live-v2
        # copyable_edge routing (+3.3% ROI backtest claim) is intentionally dropped;
        # re-introduce it only WITH these gates intact if the live push resumes.
        if quality in ("UNTRADABLE", "BAD"):
            ...
            # gap measurement.
            # Triple gate aligned with capital_allocator._elite_paper_bypass
            # and risk_engine for audit-aval coherence:
            #   1. paper_trading_enabled (paper mode)
            #   2. not live_enabled (pas live)
            #   3. not paper_live_strict (= mode permissif)
            #   4. allow_non_strict_paper_research (= opt-in research explicite —
            #      config refuse cette combo en prod/VPS)
            paper_mode = bool(
